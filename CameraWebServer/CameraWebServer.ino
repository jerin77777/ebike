...
#include "ultrasonic.h"  // HC-SR04: TRIG=GPIO12, ECHO=GPIO13
...
void setup() {
  ...
  // Ultrasonic sensor initialisation (camera code untouched above)
  ultrasonicSetup();
...
void loop() {
  ...
  // ---- Ultrasonic sensor: auto-enable stream when object < threshold ----
  {
    bool detected = ultrasonicTriggered(); // true when object < 50 cm
    if (detected && !streamingEnabled) {
      streamingEnabled = true;
      Serial.printf("[Ultrasonic] Object detected (%.1f cm) — stream ON\n", _us_lastDist);
    } else if (!detected && streamingEnabled) {
      streamingEnabled = false;
      Serial.println("[Ultrasonic] Object gone — stream OFF");
    }
  }
  // ----------------------------------------------------------------------
